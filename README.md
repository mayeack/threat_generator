...
scripts/
  ...
  validate_ta.sh          # Runs splunk-appinspect with --included-tags cloud on the tarball
                          #   Both are bash (maintainer tooling, not needed to run the app);
                          #   on Windows run them via Git Bash or WSL.
...
**Requirements**: Python 3.9+ · macOS, Windows, or Linux

Install into any user-writable folder — the SQLite database and `./logs/` output are written next to the code, so avoid system locations such as `C:\Program Files`. Secrets work on every platform automatically: the `keyring` package stores them in macOS Keychain, Windows Credential Manager, or Linux Secret Service.
...
In cmd.exe, activate with `venv\Scripts\activate` instead.
...
### Adding a HEC destination

Target stacks change regularly, and **a HEC token is only valid on the stack that issued it** — a new endpoint always needs a new token. No stack hostname is hardcoded anywhere in the app; the URL lives in the DB and is edited through Settings or `/api/hec/destinations`.

#### 1. Derive the ingest URL from the stack URL

Splunk Cloud serves HEC on a **separate hostname** from the search head. Given a stack URL, prefix the host with `http-inputs-`:

| Role | Host |
|---|---|
| Search head (what you browse / point the MCP at) | `https://<stack>.splunkcloud.com` |
| HEC ingest (what ThreatGen posts to) | `https://http-inputs-<stack>.splunkcloud.com:443` |

Sanity-check before configuring — the ingest host should resolve, typically to a load balancer:

```bash
dig +short http-inputs-<stack>.splunkcloud.com
```

Posting to the search head host instead is the most common setup failure. ThreatGen appends `/services/collector/event` automatically, so leave the path off.

#### 2. Create the index

In Splunk Web: **Settings → Indexes → New Index**, name it `threat_gen`. Skip this and the token will authenticate but events will be rejected.

#### 3. Generate the HEC token

In Splunk Web on the target stack:

1. **Settings → Data Inputs → HTTP Event Collector**.
2. **Global Settings** → confirm **All Tokens** is *Enabled* (Splunk Cloud defaults to enabled on 443).
3. **New Token** → name it (e.g. `threatgen`) → **Next**.
4. On **Input Settings**: set **Source type** to *Automatic*, add `threat_gen` to **Selected indexes**, and set **Default Index** to `threat_gen`.
5. **Review → Submit**, then copy the token value (a UUID, `8-4-4-4-12` hex).

#### 4. Store the token

Paste it into **Settings → HEC → token** in the ThreatGen UI. It is validated for shape, stored in the OS keychain, and never written to disk or echoed by any endpoint. No restart needed.

Alternatively, set `SPLUNK_HEC_TOKEN` before launching the server (`export SPLUNK_HEC_TOKEN=...` on macOS/Linux, `$env:SPLUNK_HEC_TOKEN="..."` in PowerShell) — the env var takes precedence over the keychain, so a stale keychain entry cannot shadow it.

#### 5. Point the destination and verify

```bash
curl -X PUT http://127.0.0.1:8899/api/hec/destinations/default \
  -H "Content-Type: application/json" \
  -d '{"enabled": true, "url": "https://http-inputs-<stack>.splunkcloud.com:443",
       "verify_tls": true, "default_index": "threat_gen"}'

curl -X POST http://127.0.0.1:8899/api/hec/destinations/default/test
```

Read the test result rather than assuming success — the status code localizes the fault precisely:

| Result | Meaning | Fix |
|---|---|---|
| `ok: true`, `200` | Working. | — |
| `403 Invalid token` | Reached HEC; token wrong or from another stack. | Re-issue on **this** stack (step 3). |
| `400 Incorrect index` | Token valid; index not allowed on it. | Add `threat_gen` to the token's Selected indexes. |
| DNS / connection error | Wrong hostname — usually the search head, not `http-inputs-`. | Recheck step 1. |

A `403` is good news: it proves the URL is right and only the credential is wrong.

#### 6. Restart generation

The engine snapshots HEC config when generation starts, so enabling a destination mid-run sends nothing. Restart it, then confirm `events_sent` climbs:

```bash
curl -X POST http://127.0.0.1:8899/api/generator/stop
curl -X POST http://127.0.0.1:8899/api/generator/start
curl -s http://127.0.0.1:8899/api/hec/stats
```
